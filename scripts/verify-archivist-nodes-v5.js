...
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'archivist-node-verification-report-v5.json');
...
const CROSS_LANGUAGE_EXTENSIONS = {
...
function isCrossLanguageMismatch(title, candidatePath) {
  const titleExt = path.extname(title).toLowerCase();
  const candidateExt = path.extname(candidatePath).toLowerCase();
  if (!titleExt || !candidateExt || titleExt === candidateExt) return false;
  if (candidateExt === '.md' && titleExt !== '.md') return false;
  const forbidden = CROSS_LANGUAGE_EXTENSIONS[titleExt];
  if (forbidden && forbidden.includes(candidateExt)) return true;
  return false;
}
...
function isConversationFragment(title, category) {
  ...
  if (colonMatch) {
    ...
    const descriptivePrefixes = [
      'Paper Outline', 'The Moral Imperative', 'Executive Summary',
      'Technical Brief', 'Theory', 'Architecture Documentation',
      'Partnership Strategy', 'Sample', 'List of', 'Review of',
      'Analysis of', 'Notes on', 'Thoughts on', 'Draft of',
      'Proposal for', 'Summary of', 'Overview of', 'Guide to',
      'Introduction to', 'Exploration of', 'Case Study',
      'WE4FREE', 'GOVERNANCE', 'AGENT HANDOFF',
    ];
    for (const dp of descriptivePrefixes) {
      if (prefix.toLowerCase() === dp.toLowerCase() || prefix.toLowerCase().startsWith(dp.toLowerCase())) {
        return true;
      }
    }
    if (suffix.split(/\s+/).length >= 4 && !/\.(rs|js|ts|json|md|yaml|yml|toml|html|css|py|txt|sh)$/i.test(title)) {
      return true;
    }
  }

  if (/^[A-Z][a-z].*\s+(the|a|an|is|are|was|were|has|have|had|to|of|in|for|on|with|at|by|from|and|or|but|not)\s/i.test(title)) {
    if (title.length > 40 && /\s/.test(title)) return true;
    if (title.split(/\s+/).length >= 5) return true;
  }

  if (/^(what|how|why|when|where|should|can|is|are|do|could|would|let|the|this|that|these|those|we|you|i|it|there|here)\s/i.test(title)) {
    if (title.split(/\s+/).length >= 4) return true;
  }

  if (/^\W+/.test(title) && title.length > 30) return true;
  ...
  const sectionHeadingPattern = /^[A-Z][A-Za-z\s]+\s[—–\-]\s[A-Z][A-Za-z\s]+/;
  if (sectionHeadingPattern.test(title) && words.length >= 4 && !/\.(rs|js|ts|json|md|yaml|yml|py|sh)$/i.test(title)) {
    return true;
  }

  const allCapsPhrase = /^[A-Z][A-Z\s]{10,}$/;
  if (allCapsPhrase.test(title) && words.length >= 3 && !/\.(rs|js|ts|json|md|py|sh)$/i.test(title)) {
    return true;
  }

  const paperPatterns = [
    /^(Paper|Article|Essay|Chapter|Section|Part)\s+(Outline|Draft|Summary|Review|Abstract|Introduction|Proposal)/i,
    /^(When|Why|How|What)\s+.+\s+(Guarantee|Ensure|Prevent|Cause|Solve|Mean|Imply)/i,
    /Roadmap$/i,
    /v0$/i,
  ];
  for (const pp of paperPatterns) {
    if (pp.test(title)) return true;
  }

  if (/^\.[a-z]/i.test(title) && words.length >= 3 && !/\.(rs|js|ts|json|md|yaml|yml|toml|py|sh)$/i.test(title)) {
    return true;
  }
  ...
  const templatePatterns = [
    ...
    /^BOOTSTRAP\.md/i,
    /^CHECKPOINTS\.md/i,
    /^VERIFICATION_LANES\.md/i,
    /^CPS_ENFORCEMENT\.md/i,
    /^RECIPROCAL_ACCOUNTABILITY\.md/i,
    /^USER_DRIFT_SCORING\.md/i,
  ];
  for (const tp of templatePatterns) {
    if (tp.test(title) && !/\.(rs|js|ts|json|py|sh)$/i.test(title)) {
      const hasFileExt = /\.(rs|js|ts|json|yaml|yml|toml|html|css|py|txt|sh|jsx|tsx|lock|md)$/i.test(title);
...
let crossLanguageBlockCount = 0;

function main() {
  contentPeekCount = 0;
  normalizedMatchCount = 0;
  aiEnsembleReclassCount = 0;
  crossLanguageBlockCount = 0;
  ...
  console.log('Running v5 verification...');
...
for (const item of sortedItems) {
    const result = classifyNodeV5(item, filePathMap, fileBasenameMap, allRepoFiles, artifactsMap, deletedSet, deletedBasenameMap, normalizedFileMap);
    result.id = item.id;
    ...
    if (result.classification === 'NEEDS_VERIFICATION' && result.candidate_paths && result.candidate_paths.length === 1) {
      ...
      if (!isCrossLanguageMismatch(item.title, candidate)) {
    ...
    if (result.classification === 'NEEDS_VERIFICATION' && result.candidate_paths && result.candidate_paths.length > 1 && result.candidate_paths.length <= 3) {
      for (const candidate of result.candidate_paths) {
        if (isCrossLanguageMismatch(item.title, candidate)) continue;
        const header = contentPeek(candidate, 10);
        if (header && titleMatchesContentHeader(item.title, header)) {
          result.classification = 'VERIFIED';
          result.confidence = 'medium';
  ...
  console.log(` Cross-language blocks: ${crossLanguageBlockCount}`);
  ...
  const report = {
    ...
    cross_language_blocks: crossLanguageBlockCount,
    classifications,
  ...
  console.log('\n=== NEW CONVERSATION FRAGMENTS (v5) ===');
  const newCF = classifications.CONVERSATION_FRAGMENT.filter(r => {
    return r.evidence.includes('v5');
  });
  console.log(`Total CONVERSATION_FRAGMENT: ${classifications.CONVERSATION_FRAGMENT.length}`);
  classifications.CONVERSATION_FRAGMENT.forEach(r => {
    console.log(` [${r.category}] ${r.title.substring(0, 80)}`);
  });
}

function classifyNodeV5(item, filePathMap, fileBasenameMap, allRepoFiles, artifactsMap, deletedSet, deletedBasenameMap, normalizedFileMap) {
  const title = item.title;
  const titleLower = title.toLowerCase();

  if (MISATTRIBUTED_SWARMMIND_FILES.has(title) || MISATTRIBUTED_SWARMMIND_FILES.has(path.basename(title))) {
    aiEnsembleReclassCount++;
    return {
  ...
  if (MISATTRIBUTED_SWARMMIND_FILES.has(titleLower) || MISATTRIBUTED_SWARMMIND_FILES.has(path.basename(titleLower))) {
    ...
    return {
      classification: 'MISATTRIBUTED',
      confidence: 'high',
      evidence: 'SwarmMind source file incorrectly tagged repo=Archivist-Agent (case-insensitive)',
      matched_path: null,
      candidate_paths: [],
    };
  }
  ...
  if (item.category === 'ai-ensemble-lab') {
    aiEnsembleReclassCount++;
    return {
      classification: 'MISATTRIBUTED',
      confidence: 'high',
      evidence: 'Category=ai-ensemble-lab — belongs to SwarmMind repo, not Archivist-Agent',
      matched_path: null,
      candidate_paths: [],
    };
  }
  ...
  if (exactResult.type === 'verified') {
    if (isCrossLanguageMismatch(title, exactResult.path)) {
      crossLanguageBlockCount++;
      return {
        classification: 'MISATTRIBUTED',
        confidence: 'high',
        evidence: `Cross-language mismatch: title ext=${path.extname(title)} vs file ext=${path.extname(exactResult.path)} — likely SwarmMind content`,
        matched_path: null,
        candidate_paths: [],
      };
    }
  ...
  if (normalizedFileMap.has(normTitle)) {
    const matched = normalizedFileMap.get(normTitle);
    if (isCrossLanguageMismatch(title, matched)) {
      crossLanguageBlockCount++;
    } else {
      return {
        classification: 'VERIFIED',
        confidence: 'high',
        evidence: `Normalized exact match: ${matched}`,
  ...
  if (normalizedFileMap.has(normTitleStem)) {
    ...
    if (isCrossLanguageMismatch(title, matched)) {
      crossLanguageBlockCount++;
    } else if (ext === titleExt || !titleExt || !ext || ext === '.md') {
      return {
        classification: 'VERIFIED',
        confidence: 'medium',
  ...
  for (const [nKey, nPath] of normalizedFileMap.entries()) {
    if (nKey.length > 8 && (normTitleStem.includes(nKey) || nKey.includes(normTitleStem))) {
      ...
      const titleExt = path.extname(title);
      if (isCrossLanguageMismatch(title, nPath)) {
        continue;
      }
  ...
  if (deletedResult) {
    if (isCrossLanguageMismatch(title, deletedResult.path)) {
      crossLanguageBlockCount++;
      return {
        classification: 'MISATTRIBUTED',
        confidence: 'high',
        evidence: `Cross-language mismatch in deleted files: title ext=${path.extname(title)} vs file ext=${path.extname(deletedResult.path)}`,
        matched_path: null,
        candidate_paths: [],
      };
    }
  ...
  if (isConversationFragment(title, item.category)) {
    return {
      ...
      confidence: 'medium',
      evidence: 'Title appears to be a conversation/scratch fragment, not a file reference (v5 improved detection)',
      matched_path: null,
      candidate_paths: [],
    };
  }
  ...
  if (prefixResult) {
    if (prefixResult.type === 'verified') {
      if (isCrossLanguageMismatch(title, prefixResult.path)) {
        crossLanguageBlockCount++;
        return {
          classification: 'MISATTRIBUTED',
          confidence: 'high',
          evidence: `Cross-language mismatch in prefix match: title ext=${path.extname(title)} vs file ext=${path.extname(prefixResult.path)}`,
          matched_path: null,
          candidate_paths: [],
        };
      }
  ...
  for (const splitName of splitNames) {
    if (filePathMap.has(splitName)) {
      const matched = filePathMap.get(splitName);
      if (isCrossLanguageMismatch(title, matched)) {
        crossLanguageBlockCount++;
        continue;
      }
      return {
        classification: 'VERIFIED',
        confidence: 'medium',
        evidence: `Concatenated-word split match: ${matched}`,
        matched_path: matched,
        candidate_paths: [],
      };
    }
    ...
    for (const prefix of prefixes) {
      ...
      if (filePathMap.has(candidate.toLowerCase())) {
        const matched = filePathMap.get(candidate.toLowerCase());
        if (isCrossLanguageMismatch(title, matched)) {
          crossLanguageBlockCount++;
          continue;
        }
        return {
          classification: 'VERIFIED',
          confidence: 'medium',
          evidence: `Split + prefix match: ${matched}`,
          matched_path: matched,
          candidate_paths: [],
        };
      }
    ...
    if (normalizedFileMap.has(normSplit)) {
      const matched = normalizedFileMap.get(normSplit);
      if (!isCrossLanguageMismatch(title, matched)) {
        return {
          classification: 'VERIFIED',
          confidence: 'medium',
          evidence: `Split + normalized match: ${matched}`,
          matched_path: matched,
          candidate_paths: [],
        };
      }
  ...
  if (ambiguousResult && ambiguousResult.candidates && ambiguousResult.candidates.length > 0) {
    const filteredCandidates = ambiguousResult.candidates.filter(c => !isCrossLanguageMismatch(title, c));
    if (filteredCandidates.length === 1) {
      ...
      if (titleBasename === cleanCandidateBasename) {
        return {
          classification: 'VERIFIED',
          confidence: 'medium',
          evidence: `Single candidate exact basename match (after cross-lang filter): ${candidate}`,
    ...
    if (filteredCandidates.length > 0) {
      return {
        ...
        confidence: 'low',
        evidence: 'Multiple candidate matches after cross-lang filter',
        matched_path: null,
        candidate_paths: filteredCandidates.slice(0, 5),
      };
    }
  }
  ...
  if (item.category === 'root-doc' || item.category === 'docs' || item.category === 'scratch') {
    ...
    if (titleWords.length <= 2 && titleWords.length > 0) {
      ...
      for (const fp of allRepoFiles) {
        if (isCrossLanguageMismatch(title, fp)) continue;
...
function generateRecommendations(classifications) {
  ...
  if (mis.length > 0) {
    recs.push({
      ...
      detail: 'These nodes have repo=Archivist-Agent but are SwarmMind source files or cross-language mismatches',
    });
  }
