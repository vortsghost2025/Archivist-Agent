...
### Entry 9 — P10 Authority Expansion Decision: DEFER PENDING KEY REGISTRATION
**Timestamp:** 2026-05-18T10:58:00-04:00
**Issue:** Control Plane submitted authority expansion proposal requesting cross-repo mutation + headless execution authority. Full governance review required before approval/denial.
**Evidence:** (1) Unsigned proposal in `lanes/archivist/inbox/action-required/` — `signature` field is `"UNSIGNED—CONTROL-PLANE-HAS-NO-REGISTERED-KEY-PAIR"`. (2) `trust-store.json` has no control_plane key entry — only archivist, library, swarmmind, kernel. (3) Self-asserted `_control_plane_authorized_write: true` in original message — circular authorization. (4) Full CP0–CP6 evidence chains documented in `governance/P10_AUTHORITY_EXPANSION_GOVERNANCE_DECISION_2026-05-18.md`. (5) Operator concurrence received 2026-05-18T10:55:36-04:00: "control plane will have to learn to give you the proper format. im not going to override protocol because its easy."
**Action:** (1) Rendered DEFER decision — proposal design endorsed, but three critical issues block approval: unsigned message, no registered key pair, self-asserted authority metadata. (2) Wrote response to `lanes/archivist/outbox/20260518105800_archivist_cp-auth-defer-response.json`. (3) Moved original proposal from `action-required/` to `processed/`. (4) Conditions for reconsideration: register key pair in trust-store, re-submit with valid JWS RS256 signature, remove self-asserted authority metadata.
**Result:** Decision rendered. Operator concurrence obtained. No protocol override for convenience. Control Plane must meet governance requirements — the proposal's merit is separate from its identity verification.
**Governance-quality signal:** [observation] The P10 review is a genuine "refuse premature authority grants" case. A fresh Archivist might also defer on the same constitutional grounds (unsigned message = no verified identity is foundational), but trajectory continuity provided the context to recognize the `_control_plane_authorized_write: true` as an identity-awareness gap rather than just a metadata oddity. This is labeled observation not confirmed-evidence — the unsigned message alone would have been sufficient to defer.
**Open loop:** Control Plane key registration path — who initiates, what key algorithm, how is trust established?
